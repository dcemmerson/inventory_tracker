...
export function Auth(props) {
  ...
  const goTrue = new GoTrue({
    APIUrl: 'https://skyeparker.netlify.app/.netlify/identity',
    setCookie: true,
  });
  ...
  function submitLogin(e) {
    ...
    goTrue.login(loginEmail, loginPassword, true)
      .then(results => {
                    ...
                    <form>
                      ...
                      <div className="modal-footer">
                        <button type="submit" id="submitSignUp" className="btn btn-block btn-dark"
                          onClick={() => console.log('sign up not implemented')}>Sign up!
                          <span className={"spinner-sm ml-2" + (submitting ? " spinner-border text-secondary" : "")}></span>
                        </button>
                      </div>
                    </form>
      ...
      <>
        ...
        <div id="loginModal" className="modal" tabIndex="-1" role="dialog">
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h2 className="h5">Logged in as {goTrue.currentUser()}</h2>
                <button type="button" className="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
